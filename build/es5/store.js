...
export function buildStore(storeHistory, reducersMapObject, storeMiddlewares, storeEnhancers, initData, routerParser) {
    ...
    var effectMiddleware = function (_a) {
        ...
        return function (next) { return function (originalAction) {
            ...
            var handlersEvery = store.reactCoat.effectMap[action.type.replace(new RegExp("[^" + NSP + "]+"), "*")] || {};
            var handlers = tslib_1.__assign({}, handlersCommon, handlersEvery);
            var handlerModules = Object.keys(handlers);
            if (handlerModules.length > 0) {
    ...
    var enhancers = [applyMiddleware.apply(void 0, [effectMiddleware, routerMiddleware(storeHistory)].concat(storeMiddlewares))].concat(storeEnhancers);
    ...
    store = createStore(combineReducers, initData, compose.apply(void 0, enhancers));
    if (store.reactCoat) {
